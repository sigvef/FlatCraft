...
public class Inventory {
	...
	public void render(Graphics g) {
		...
		g.translate((Main.SCREEN_W / 2f) - 9 * Main.GULOL, Main.SCREEN_H - 3
				* Main.GU - 7*Main.GULOL);
		g.setColor(new Color(0, 0, 0, 0.5f));
		g.fillRoundRect(0, 0, 20 * Main.GULOL, 6 * Main.GULOL, 15);

		g.setColor(Color.lightGray);
		g.fillRect(4 * Main.GULOL, 1 * Main.GULOL, 3 * Main.GULOL,
				3 * Main.GULOL);

		g.setColor(Color.pink);
		g.fillRect(9 * Main.GULOL, 1 * Main.GULOL, 3 * Main.GULOL,
				3 * Main.GULOL);

		g.setColor(new Color(0f, 0.6f, 1, 0.4f));
		g.fillRect(14 * Main.GULOL, 1 * Main.GULOL, 3 * Main.GULOL,
				3 * Main.GULOL);

		g.setColor(Color.black);
		if (activeElement == Block.ROCK) {
			g.setColor(Color.white);
			g.drawString(inventory.get(Block.ROCK).toString(), 5 * Main.GULOL,
					3 * Main.GULOL);
			g.setColor(Color.black);
		} else {
			g.drawString(inventory.get(Block.ROCK).toString(), 5 * Main.GULOL,
		...
		if (activeElement == Block.RUBBER) {
			g.setColor(Color.white);
			g.drawString(inventory.get(Block.RUBBER).toString(),
					10 * Main.GULOL, 3 * Main.GULOL);
			g.setColor(Color.black);
		} else {
			g.drawString(inventory.get(Block.RUBBER).toString(),
					10 * Main.GULOL, 3 * Main.GULOL);
		...
		if (activeElement == Block.WATER) {
			g.setColor(Color.white);
			g.drawString(inventory.get(Block.WATER).toString(),
					15 * Main.GULOL, 3 * Main.GULOL);
			g.setColor(Color.black);
		} else {
			g.drawString(inventory.get(Block.WATER).toString(),
					15 * Main.GULOL, 3 * Main.GULOL);
